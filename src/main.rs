...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if config.a3_1.len() != config.a3_2.len() || config.a3_1.len() !=config.a3_3.len() ||config.a3_1.len() !=config.a3_name.len() || config.a3_1.len() != config.a3_path.len(){
        println!("a3开头的变量不完整会导致事件无法触发");
        a3_a =1;
    }
    if config.b4_1.len() != config.b4_2.len() ||config.b4_1.len() != config.b4_3.len() ||config.b4_1.len() != config.b4_4.len() ||config.b4_1.len() != config.b4_name.len() ||config.b4_1.len() != config.b4_path.len(){
        println!("b4开头的变量不完整会导致事件无法触发");
        b4_b =1;
    } 
    if config.c5_1.len() != config.c5_2.len() ||config.c5_1.len() != config.c5_3.len() ||config.c5_1.len() != config.c5_4.len() ||config.c5_1.len() != config.c5_5.len() ||config.c5_1.len() != config.c5_name.len() ||config.c5_1.len() != config.c5_path.len() {
        println!("c5开头的变量不完整会导致事件无法触发");
        c5_c =1;
    }
    if config.d_file1_json.len() != config.d_file2_json.len() ||config.d_file1_json.len() != config.d_file3_json.len() ||config.d_file1_json.len() != config.d_file4_json.len() ||config.d_file1_json.len() != config.d_file5_json.len() ||config.d_file1_json.len() != config.d_file6_json.len() ||config.d_file1_json.len() != config.d_file_name.len() ||config.d_file1_json.len() != config.d_file_path.len(){
        d6_d =1;
        println!("d_file开头的变量不完整会导致事件无法触发");
    } 
